    ...
    async setInteract(uid, list) {
        ...
        for (let item of list) {
            ...
            try {
                ...
                const result = await app.mysql.query(sql, [collect, thumbsUp, pid]);
                if (collectActive) {
                    let res = await app.mysql.insert('post_collects', { postid: pid, userid: uid })
                    console.log(res);
                }
                if (likeActive) {
                    await app.mysql.insert('post_likes', { postid: pid, userid: uid })
                }
                // console.log(result);
            } catch (err) {
